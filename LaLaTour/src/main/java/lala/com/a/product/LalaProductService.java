...
public interface LalaProductService {
	...
	//제품
	public int productWriteAf(ProductDto dto); //제품등록
	...
	public List<GoodsDto> getGoodsPagingList(PagingBean pagingBean); //상품평 페이징목록
	...
	//과거 주문내역
	public List<OrderedDto> getSellList(PagingBean pagingBean); //내역리스트(상품평도 있음)
	public List<OrderedDto> getOrderSList(String omid); //내역리스트 - 특정주문번호
	public int getSellListTotalCount(String id); //페이징 위해 총 갯수 가져옴
	...
	public GoodsDto goodsDetail(GoodsDto dto); //상품평 수정을 위해 상품평 디테일 가져가기
	public boolean updateGoodsAf(GoodsDto dto); //상품평 수정
	public boolean productPointMinus(GoodsDto dto); //상품평 수정시 총별점에서 원본별점 -
	public boolean productPointPlus(GoodsDto dto); //상품평 수정시 총별점에 수정된별점 +
	
	public int getGoodsTotalCount(PagingBean pagingBean); //상품평 갯수 얻기
}
